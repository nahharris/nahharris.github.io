...
This work is about adapting the Actor Model of Computation to non-distributed systems, the idea is to produce a framework that can be used to build centralized applications in [Rust](https://rust-lang.org/). The proof of concept is building a centralized application based on [patch-hub](https://github.com/kworkflow/patch-hub) using the [Tokio](https://tokio.rs/) async runtime.
